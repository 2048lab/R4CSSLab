...
# Getting essential words by tf-idf
## Calculating tf-idf
```{r}
library(tidytext)
tf_idf <- unnested.df %>%
...
## filtered tf_idf words
```{r}
tf_idf_words <- tf_idf %>%
    filter(log10(tf_idf) > median(log10(tf_idf)) + 2*sd(log10(tf_idf))) %>%
    select(word, tf_idf) %>%
    filter(!duplicated(.))
```
...
word_corr %>%
    ...
    theme_void()
```
## counts of pair of words
```{r}
word_count <- unnested.df %>%
    group_by(word) %>%
    filter(n() > 20) %>%
    ungroup() %>%
...
word_count %>%
    ...
    geom_node_point(color = "lightblue", size = 2) +
    geom_node_text(aes(label = name), repel = F, family = "Heiti TC Light", color="blue") +
    theme_void()
```


## filter word by tf_idf words
```{r}
word_corr %>%
    filter(item1 %in% tf_idf_words$word) %>%
    filter(item2 %in% tf_idf_words$word) %>% View
    filter(correlation > 0.33) %>%
    graph_from_data_frame() %>%
    # ggraph(layout = "fr") +
    ggraph(layout = "kk") +
...
```{r}
V(g)$degree.centralization <- centralization.degree(g)$res
# V(g)$degree.centralization

length(V(g)) # 478
test <- eigen_centrality(g)
...
```{r}
cluster1 <- cluster_louvain(g)
cluster2 <- cluster_edge_betweenness(g)
```
...
# for tree-based hierarchical map
is_hierarchical(cluster1)
# merges(cluster1)
# code_len(cluster1)
# as_phylo(cluster1)
...
plot(cluster1, g, 
     col = membership(cluster1),
     mark.groups = communities(cluster1), 
     edge.color = c("black", "red")[crossing(cluster1, g) + 1],
     vertex.label.family = 'Heiti TC Light')
```
...
```{r}
clu <- components(g)
g.sub <- induced_subgraph(g, groups(clu)$`1`)
sub.clu <- cluster_louvain(g.sub)
plot_cluster <- function(x, y){
    plot(x, y, 
     col = membership(x),
...
plot_cluster(sub.clu, g.sub)
```


```{r}
netm <- get.adjacency(g.sub, 
                      attr="weight", 
                      sparse=F)

palf <- colorRampPalette(c("gold", 
                           "dark orange")) 

par(family = "Heiti TC Light")
heatmap(netm[1:30,1:30], 
        Rowv = NA, Colv = NA, 
        col = palf(100),
        scale="none", 
        margins=c(10,10))
